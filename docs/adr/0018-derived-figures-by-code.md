...
The pin is deterministic and append-only, so it ships on rather than waiting for a measured run; `PIN_DERIVED_INPUTS=off` restores the pre-#287 behaviour for the comparison.
