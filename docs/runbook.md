...
```
tramitico.event {"event":"ask","outcome":"ok","latency":"1s_3s","stages":{"condense":"lt_1s","retrieve":"lt_1s","rerank":"lt_1s","generate":"1s_3s","validate":"lt_1s","persist":null},"generations":[{"latency":"1s_3s","firstText":"lt_1s","cache":"read"}],"providerError":null,"citationFailure":false,"quotaHit":false,"quotaReason":null,"abort":null,"routedCategory":null}
```
...
|                   | `charged_error`                                                                                                                                       | **our side broke** (or the search rejected the request's own text), but paid work had run, so the ask kept its charge: the deadline expired after generation began, or `retrieval_failed` with a SQLSTATE in class 22/54 (`providerError` names it)                                                                                                                                                                                       |
...
|     | … and those that kept their charge    | `charged_error` — a deadline past the first generation, or a search the request's own text broke (ADR 0013's amendment)                                                                            |
...
codebase, so those queries need no quoting. Q3, Q4, Q6, Q8 and Q9 match on JSON fragments;
