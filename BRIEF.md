# Contadito — Project Brief *(working name — will be renamed)*
...
Why RAG specifically — and why *this* corpus:
...
2. **An eval harness.** A curated Q&A eval set plus an automated groundedness check
   ...
   never show that the author *evaluates* their RAG; this one does, in CI.
...
upgrades the public skill claim from *"integrates LLMs"* to *"builds agentic tools / MCP
servers"* — it must actually ship before that claim is ever made.
...
- [ ] Corpus slice: *which* 8–15 documents? What are the actual top-10 highest-pain questions?
